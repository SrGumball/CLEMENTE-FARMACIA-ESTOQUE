  ...
  const estoquePorMedicamento = medicamentos
    ...
    .map(med => {
      const lotesDoMed = lotes
        ...
        .map(l => {
          const vencido = parseISO(l.data_validade) < hoje;
      ...
      return {
        ...med,
        lotes: lotesDoMed,
        isVelho: med.created_at ? differenceInDays(hoje, parseISO(med.created_at)) > 180 : false,
        ...
        if (med.lotes.length === 0) {
          ...
          med.lotes.forEach(lote => {
            rows.push([
              ...
              med.unidade_medida || "-",
              lote.numero_lote,
              format(parseISO(lote.data_validade), "dd/MM/yyyy"),
